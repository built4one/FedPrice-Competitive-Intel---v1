...
export default function IntakeNode({ onBack, onSuccess }: Props) {
  ...
  const chooseFiles = (newFiles?: FileList | null) => {
    ...
    for (const f of added) {
      if (f.size > 25 * 1024 * 1024) return setError(`File ${f.name} must be 25 MB or smaller.`);
    }
    setError('');
    ...
    {!processing ? <>
      ...
      <div className="mt-8 max-w-2xl">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); chooseFiles(event.dataTransfer.files); }} onClick={() => inputRef.current?.click()} className={`grid min-h-64 cursor-pointer place-items-center rounded-xl border-2 border-dashed px-6 text-center transition ${dragging ? 'border-blue-500 bg-blue-50' : files.length > 0 ? 'border-emerald-400 bg-emerald-50/40' : 'border-slate-300 bg-slate-50/50 hover:border-blue-400'}`}>
            ...
            <div>
              ...
              <p className="mt-2 text-xs leading-5 text-slate-500">PDF, DOCX, DOC, or TXT · up to 25 MB</p>
            ...
            <div className="mt-4 space-y-2">
              {files.map((f, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2 text-sm bg-slate-50">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); removeFile(i); }} className="p-1 hover:bg-slate-200 rounded-md text-slate-500">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              ))}
