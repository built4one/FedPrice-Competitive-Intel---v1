...
export function calculateCompanyPosition(market: MarketPosition, company: CompanyContext): CompanyPosition {
  ...
  if (!market.low || !market.high || !market.target) return { effectivePrice: price, impliedMarginPct: company.costBaseline ? ((price - company.costBaseline) / price) * 100 : undefined, bandPosition: 'MARKET_RANGE_UNAVAILABLE', fitScore: 0, assessment: 'Market position is not numerically supportable.', recommendedAction: 'Focus on non-price factors.' };
  
  const deltaToMarketPct = ((price - market.target) / market.target) * 100;
  ...
  if (price < market.low) bandPosition = 'BELOW_MARKET';
  else if (price > market.high) bandPosition = 'ABOVE_MARKET';
